...
#[derive(AnchorSerialize, AnchorDeserialize)]
pub struct BatchVerifyNGOsParams {
    pub ngo_authorities: Vec<Pubkey>,
    pub reason: String,
}

#[derive(Accounts)]
#[instruction(params: BatchVerifyNGOsParams)]
pub struct BatchVerifyNGOs<'info> {
    #[account(
        seeds = [b"config"],
        bump = config.bump,
        constraint = config.admin == admin.key() @ ErrorCode::UnauthorizedAdmin
...
pub fn batch_verify_ngos_handler<'info>(
    ...
    for ngo_authority in params.ngo_authorities.iter() {
        if let Some(ngo_account_info) = remaining_accounts.next() {
            let (expected_pda, _) =
                Pubkey::find_program_address(&[b"ngo", ngo_authority.as_ref()], ctx.program_id);

            if ngo_account_info.key() != expected_pda {
                msg!("Skipping invalid NGO account: {}", ngo_authority);
                skip_count += 1;
                continue;
            }

            let mut ngo: Account<NGO> = Account::try_from(ngo_account_info)?;

            if ngo.is_blacklisted {
                msg!("Skipping blacklisted NGO: {}", ngo.name);
                skip_count += 1;
                continue;
            }
            ...
            ngo.exit(ctx.program_id)?;

            success_count += 1;
            msg!("Verified NGO: {}", ngo.name);

            if let Some(admin_action_info) = remaining_accounts.next() {
                let mut admin_action: Account<AdminAction> = Account::try_from(admin_action_info)?;

                admin_action.action_type = AdminActionType::VerifyNGO;
                admin_action.target = ngo.key();
                admin_action.admin = ctx.accounts.admin.key();
                admin_action.reason = params.reason.clone();
                admin_action.timestamp = clock.unix_timestamp;
                admin_action.metadata = String::new();

                admin_action.exit(ctx.program_id)?;
            }
        } else {
            msg!("Missing account for NGO: {}", ngo_authority);
            skip_count += 1;
        }
    }

    msg!("Batch verification complete");
    msg!("Successful: {}", success_count);
    msg!("Skipped: {}", skip_count);

    Ok(())
}

#[derive(AnchorSerialize, AnchorDeserialize)]
pub struct BatchUpdateNGOStatusParams {
    pub ngo_authorities: Vec<Pubkey>,
    pub is_active: bool,
    pub reason: String,
}
...
#[derive(Accounts)]
#[instruction(params: BatchUpdateNGOStatusParams)]
pub struct BatchUpdateNGOStatus<'info> {
    #[account(
        seeds = [b"config"],
        bump = config.bump,
        constraint = config.admin == admin.key() @ ErrorCode::UnauthorizedAdmin
...
pub fn batch_update_ngo_status_handler<'info>(
    ...
    msg!("Batch status update complete");
    msg!("Successful: {}", success_count);
    msg!("Skipped: {}", skip_count);
